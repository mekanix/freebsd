...
typedef int32_t sample_t;
int format = AFMT_S32_NE; /* Not a real value, just silencing compiler errors */
#endif
...
oss_init(config_t *config)
{
  ...
  if (config->frag < min_frag) { config->frag = min_frag; }

  /*
   * Allocate buffer in fragments. Total buffer will be split in number
   * of fragments (2 by default)
   */
  if (config->buffer_info.fragments < 0) { config->buffer_info.fragments = 2; }
